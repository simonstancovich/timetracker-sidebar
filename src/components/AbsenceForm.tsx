...
import { HOURS_PER_WORKDAY, workingDaysInRange } from "../lib/absence";
...
export function AbsenceForm({
  ...
  const headerHint = company ? company.name : t("absence.noClient");

  if (!company) {
    return (
      <Page title={t("absence.title")} hint={headerHint}>
  ...
  const canSubmit = !!project && note.trim().length > 0 && hasDays && !saving;
    ...
    <Page title={t("absence.title")} hint={headerHint}>
      <prim.FormCard>
        ...
        <prim.MonoText
          ...
          tracking="wider"
          align="center"
          color={hasDays ? "accent" : "faint"}
        >
          {hasDays
            ? t("absence.preview", {
                days: days.length,
                hours: days.length * HOURS_PER_WORKDAY,
              })
            : t("absence.noDays")}
        </prim.MonoText>
        <prim.Stack direction="row" justify="end" gap="sm">
          ...
          <prim.Button
            ...
            size="sm"
            disabled={!canSubmit}
            onClick={() => onSubmit(project, effectiveFrom, to, note)}
          >
